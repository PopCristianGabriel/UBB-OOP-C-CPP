...
int buttonExample(int argc, char *argv[])
{
	...
	QIcon icon("C:\\Users\\Pop\\source\\repos\\QtGuiApplication4\\QtGuiApplication4\\Peacock.JPG");
	button.setIcon(icon);
	//button.adjustSize();
	
	button.setIconSize(QSize(1000, 1000));
...
int gridLayout(int argc, char *argv[])
{
	...
	gridLayout->addWidget(new QPushButton("Sort"), 2, 0); // row span = 1, column span = 3
...
int createAbsolute(int argc, char *argv[])
{
	...
	lbl->setGeometry(100, 10, 40, 20);
	QLineEdit* txt = new QLineEdit(main);
	txt->setGeometry(60, 10, 10, 10);
	main->show();
