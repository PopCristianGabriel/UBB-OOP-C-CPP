...
void UI::save_into_file()
{
	...
	if (!(strcmp(this->fileType, "csv"))) {
		FileClass* file = new CSVFile(this->myListFilePath);
		file->write_to_file(this->pointerController->get_saves());
	}

	else {
		FileClass* file = new HTMLFile(this->myListFilePath);
		file->write_to_file(this->pointerController->get_saves());
	}
}
		...
		if (strcmp(userInput, "mode B") == 0) {
		 ...
		 if (strcmp(command, "add") == 0) {
			...
			try {
				...
				cout << ex.get_file() << " " << ex.get_func() << " " << ex.get_info() << " \n" << ex.get_line();
			}
